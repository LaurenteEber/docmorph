...
The current mock evidence harness requires `--manifest`, `--receipt-dir`, `--catalog`, and `--repository-root`. From a clean checkout, run the same manifest twice with two distinct fresh receipt directories and validate `fixtures/corpus-manifest.json`; the generated `semantic_sha256` values must match. Compare a generated hash with the matching retained receipt only when `build_compiler` and `platform` provenance match. A provenance mismatch makes no retained-equality claim and does not regenerate retained evidence. Rust `1.96.0` is the exclusive author of retained baselines when deliberate regeneration is required; stable and CI are validation-only and must never silently rewrite committed retained evidence or introduce a write path. A successful governed receipt uses schema `1.2`; its `catalog_id` and `catalog_revision_sha256` bind it to the validated catalog and its canonical execution revision.
...
### Named synthetic run
`synthetic-smoke` is the only governed named workflow: it executes mock-text `policy-failure` then `success` in a fresh run root and publishes one aggregate report. Named outcomes use global precedence `3 > 5 > 4 > 0`: exit `0` is a complete comparable match; exit `2` is a write-free preflight/configuration rejection; exit `3` is an operational or report-persistence failure after root acquisition; exit `4` is a comparable mismatch with no higher-precedence outcome; and exit `5` is an incomparable environment with no operational failure. The report preserves every ordered case record and its independent state, so an incomparable case remains visible when a separate operational failure produces exit `3`. Neither outcome selects a PDF engine or claims PDF fidelity. This adds no receipt-1.2 migration: retained receipts stay comparison inputs, Rust `1.96.0` exclusively authors retained baselines, and stable/CI validate only without silently rewriting them.
