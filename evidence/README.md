...
|---|---|---|
| `success/receipt.json` | `success` | Provenance, hashes, measured elapsed time, and `artifacts/success-output.mock`. |
| `policy-failure/receipt.json` | `failure` | Provenance, hashes, policy diagnostic, and `artifact: null`. No output artifact is retained. |
...
| `semantic_sha256` | A SHA-256 identity over deterministic receipt fields. It excludes elapsed time but includes the metric availability state. |
